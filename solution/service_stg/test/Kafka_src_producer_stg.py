# Это продьюсер, который отправляет данные на сервер в топик order-service_orders
# Он используется для создания данных, если в учебном сервере нет ничего
...
producer.send('order-service_orders', json.dumps(data).encode('utf-8'), b'16789012kj1g1h')
producer.flush()
producer.close()